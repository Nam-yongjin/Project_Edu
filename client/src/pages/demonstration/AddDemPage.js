...
const AddDemPage = () => {
    // input 목록을 id 배열로 관리 (빈 파일 상태로 추가) [Date.now()] 이니
    ...
    const handleFileChange = (id, e) => {
        ...
        const newPreviews = files.map((file) => ({
            file,
            url: URL.createObjectURL(file), // 브라우저가 임시로 생성하는 로컬 url
            name: file.name,
        }));
        setImages((prev) => [...prev, ...newPreviews]);

        if (fileInputs[fileInputs.length - 1] === id) {
            setFileInputs((prev) => [...prev, Date.now()]); // state의 세터 함수의 인자로는 이전 상태 값이 저장된다.      
            //  여기선 이전값을 인자로 가져와서 이전값+현재값이 추가되는 형태
        }
    };
    ...
    const addDem = () => {
        ...
        formData.append("expDate", dem.expDate)
    }
    ...
    // e.target.value가 문자열이 아닌 date타입이므로
    // 따로 구현
    ...
    return (
        <div>
            ...
            <div className="flex mt-10 ml-[230px]">
                <div className="space-y-6 w-2/3">
                    ...
                    <div className="flex items-start">
                        <label className="text-xl font-semibold w-[120px] pt-3">반납 날짜:</label>
                        <DatePicker
                            className="border p-3 text-lg flex-1 resize-y"
                            selected={returnDate}
                        ...
                        {fileInputs.map((id, index) => (
                            <div key={id} className="flex items-center mt-3">
                                ...
                                <input
                                    type="file"
                                    multiple
                                    accept="image/*"  /* 파일 확장자 지정*/
                                    onChange={(e) => handleFileChange(id, e)}
                                    className="border p-2 text-base flex-1 cursor-pointer" />
                            </div>
                        ))}

                        {/* 버튼은 input 아래에 위치 */}
                        <div className="mt-4 flex justify-end gap-4 pr-2">
                            <button className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 shadow" onClick={addDem}>
                                상품 등록
                            </button>
