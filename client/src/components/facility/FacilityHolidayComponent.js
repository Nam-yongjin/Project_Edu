...
// 백엔드 HolidayReason enum과 동일한 옵션
...
export default function FacilityDetailComponent() {
  ...
  const submitHoliday = async () => {
    ...
    try {
      setSaving(true);
      await addPublicHoliday({ date: selectedDate, label });
  ...
  return (
    ...
    <div className="max-w-screen-xl mx-auto my-10">
      ...
      <div className="min-blank">
        ...
        {/* 달력 카드 영역 */}
        <div className="page-shadow rounded-2xl border bg-white p-4">
          {/* 월 이동 바 */}
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="newText-xl font-bold">{monthLabel}</div>
            <button
          ...
          {/* 요일 헤더 */}
          <div className="grid grid-cols-7 text-center newText-sm font-medium mb-2">
            {["일","월","화","수","목","금","토"].map((d, i) => (
              <div
                key={i}
                className={i===0 ? "text-red-500" : i===6 ? "text-blue-500" : "text-gray-700"}
              >
                {d}요일
              </div>
            ))}
              ...
              week.map((d, di) => {
                ...
                return (
                  <button
                    ...
                    key={`${wi}-${di}`}
                    className={[
                      "text-left bg-white min-h-[96px] p-2 newText-sm relative focus:outline-none",
                  ...
                  >
                    <div
                      className={[
                        "absolute top-2 right-2 newText-xs",
                    ...
                    <div className="mt-6 flex flex-col gap-1">
                      {items.map((it, idx) => (
                        <span
                          key={idx}
                          className={[
                            "inline-block w-fit newText-xs px-2 py-0.5 rounded-full",
                            it.type === "PUBLIC"
        ...
        {/* 상태 메시지 */}
        {loading && <div className="mt-3 newText-sm text-gray-500">휴무일 불러오는 중…</div>}
        {saving && <div className="mt-2 newText-sm text-gray-500">처리 중…</div>}
        ...
        {modalOpen && (
          <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" role="dialog" aria-modal="true">
            <div className="w-full max-w-md rounded-2xl bg-white p-5 page-shadow">
              ...
              <div className="mt-3 newText-base text-gray-700">
                날짜: <span className="font-semibold text-gray-900">{selectedDate}</span>
              </div>

              <div className="mt-4">
                <label className="block newText-sm font-medium mb-2">사유 선택</label>
                <div className="grid grid-cols-1 gap-2">
                  {HOLIDAY_REASON_OPTIONS.map((opt) => (
                    <label key={opt.code} className="flex items-center gap-2 p-2 rounded border hover:bg-gray-50 cursor-pointer">
                      ...
                      />
                      <span className="newText-base">{opt.label}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="mt-5 flex justify-end gap-2">
                <button
                  className="normal-button newText-base px-4 py-2 rounded"
                  onClick={() => setModalOpen(false)}
                  disabled={saving}
                  type="button"
                >
                ...
                <button
                  className="positive-button newText-base px-4 py-2 rounded disabled:opacity-50"
