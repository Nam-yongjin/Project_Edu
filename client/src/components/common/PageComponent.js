...
const PageComponent = ({ totalPages, current, setCurrent }) => { 
    ...
    return (
        <div className="flex gap-x-2 items-center">
                {startPage>0 ? <div><button onClick={()=>setCurrent((currentGroup-1)*10)}>&lt;</button></div> : <div></div>} 
            ...
            {Array.from({ length: endPage - startPage }).map((_, i) => {
                ...
                {currentGroup!==Math.floor(totalPages/10) ? <div><button onClick={()=>setCurrent((currentGroup+1)*10)}>&gt;</button></div> : <div></div>}
                {currentGroup!==Math.floor(totalPages/10) ?<div><button onClick={()=>setCurrent((totalPages/10)*10)}>&gt;&gt;</button></div> : <div></div>}
        </div>
    );
};
