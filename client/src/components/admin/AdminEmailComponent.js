...
const AdminEmailComponent = ({ members }) => {
  ...
  useEffect(() => {
    ...
    // Quill 스타일 설정 - 600px 고정 크기로 이메일 템플릿과 맞춤
    ...
    style.innerHTML = `
      ...
      .ql-editor img { 
        max-width: 560px !important; /* 패딩 40px 고려 */
        width: 300px; /* 기본 크기 */
        height: 300px; /* 기본 높이 */
  ...
  const setupImageResizing = (quill) => {
    ...
    images.forEach((img) => {
      ...
      if (!img.style.width) {
        ...
        img.dataset.customWidth = "300px";
      }
      if (!img.style.height) {
        img.style.height = "300px";
        img.dataset.customHeight = "300px";
      }
      img.style.display = "inline-block";
      img.style.maxWidth = "560px"; // 이메일 템플릿에 맞는 최대 크기
      img.style.cssText += '; max-width: 560px !important;';
  ...
  const applyImageSize = () => {
    ...
    // 이메일 템플릿에 맞는 최대 크기로 수정 (560px)
    if (width && (width < 100 || width > 560)) {
      alert("너비는 100px에서 560px 사이의 값을 입력해주세요.");
      return;
    }

    if (height && (height < 100 || height > 400)) {
      alert("높이는 100px에서 400px 사이의 값을 입력해주세요.");
      return;
    }
    ...
    if (selectedImage) {
      if (width > 0 && width <= 560) {
        ...
        selectedImage.dataset.customWidth = width + "px";
      }
      if (height > 0 && height <= 400) {
        selectedImage.style.height = height + "px";
        selectedImage.dataset.customHeight = height + "px";
      } else if (!height) {
        selectedImage.style.height = "auto";
        selectedImage.dataset.customHeight = "auto";
      }

      selectedImage.style.maxWidth = "560px";
      selectedImage.style.cssText += '; max-width: 560px !important;';
    }
  ...
  const formats = [
    ...
    "color", "background", "list", "bullet", "link", "image", "align", "indent",
  ];
  ...
  const handleAttachFiles = (e) => {
    ...
    if (attachFiles.length + files.length > 6) {
      alert("첨부파일은 최대 6개까지 업로드할 수 있습니다.");
  ...
  // 공백 보존 처리 함수
  const preserveWhitespace = (htmlContent) => {
    // 연속 공백을 &nbsp;로 변환
    return htmlContent.replace(/ {2,}/g, (match) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // 공백 보존 처리
      finalContent = preserveWhitespace(finalContent);
            ...
            <div className="p-6">
              ...
              <div className="mb-6">
                ...
                <div className="newText-sm text-gray-600 mb-2">
                  이미지를 더블클릭하면 크기를 조절할 수 있습니다. Tab 키로 들여쓰기가 가능합니다.
                </div>
                  ...
                  <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full mx-4">
                    ...
                    <div className="mb-4">
                      ...
                      <input
                        ...
                        className="input-focus w-full"
                        placeholder="예: 300 (비워두면 자동)"
                      />
                      <div className="newText-xs text-gray-500 mt-1">
                        너비: 100px ~ 560px, 높이: 100px ~ 400px (이메일 최적화)
                      </div>
                    </div>
              ...
              <div className="text-center pt-4 ">
                <button
                  ...
                  className={`newText-base px-6 py-3 rounded-lg font-semibold transition-colors ${isUploading
                    ? 'disable-button'
                    : 'positive-button'
                    }`}
                >
