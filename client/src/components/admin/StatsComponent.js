...
import { getVisitorStats, getMemberRoleStats, getEventCategoryStats, getFacTimesStats, getDemRegStats, getDemRevStats } from "../../api/adminApi";
...
const StatsComponent = () => {
    ...
    const [demReg, setDemReg] = useState({ total_demReg: 0, trend: [] });
    const [demRev, setDemRev] = useState({ total_demRev: 0, trend: [] });
    ...
    useEffect(() => {
        ...
        getDemRegStats().then(setDemReg)
            .catch(error => {
                console.error("실증등록내역을 불러올 수 없습니다.:", error);
            });
        getDemRevStats().then(setDemRev)
            .catch(error => {
                console.error("실증대여내역을 불러올 수 없습니다.:", error);
            });
    }, []);
    ...
    return (
        <div className="max-w-screen-xl mx-auto my-10 text-center">
            <div className="min-blank ">
                ...
                {/* 공간 대여 인기 시간대 */}
                <div className="my-10 overflow-x-auto border-b border-gray-300 pb-4">
                    ...
                    <div className="flex flex-wrap justify-center overflow-x-auto">
                        <div className="page-shadow pt-10">
                            ...
                            <Plot
                                data={[
                                    {
                                        labels: demReg.trend.map(item => item.category),
                                        values: demReg.trend.map(item => item.count),
                                        type: "pie",
                                        textinfo: "label+percent",
                                    },
                                ]}
                                layout={{ title: "실증 등록 수 비율" }}
                            />
                        </div>
                        <div className="page-shadow my-10 pt-10">
                            <div className="newText-xl font-semibold">
                            ...
                            <Plot
                                data={[
                                    {
                                        labels: demRev.trend.map(item => item.category),
                                        values: demRev.trend.map(item => item.count),
                                        type: "pie",
                                        textinfo: "label+percent",
                                    },
                                ]}
                                layout={{ title: "실증 대여 수 비율" }}
                            />
                        </div>
                    </div>
                </div>
