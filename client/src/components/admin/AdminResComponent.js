...
const AdminResComponent = () => {
    ...
    const handleRental = async (demRegNum, state) => {
        try {
            alert(state === "ACCEPT" ? "대여 신청이 수락되었습니다." : "대여 신청이 거부되었습니다.");
            await updateResState(demRegNum, state);
                    ...
                    <table className="min-w-full border-collapse table-fixed">
                        ...
                        <tbody className="text-gray-600 text-xs">
                            {resInfo.content.length === 0 ? (
                                ...
                                resInfo.content.flatMap((member) => {
                                    const mainImage = member.imageList?.find((img) => img.isMain === true);

                                    // 요청이 없으면 기본 행 하나
                                    if (!member.requestDTO || member.requestDTO.length === 0) {
                                        return (
                                            <tr key={member.demRevNum} className="border-b hover:bg-gray-50">
                                                ...
                                                <td className="py-1 px-2 font-semibold whitespace-nowrap text-center">{getStateLabel(member.state)}</td>
                                                ...
                                                <td className="py-1 px-2 whitespace-nowrap text-center">-</td>
                                            </tr>
                                        );
                                    }

                                    // 요청이 있으면 요청별로 행 생성
                                    return member.requestDTO.map((req, idx) => (
                                            ...
                                            <td className="py-1 px-2 whitespace-nowrap text-center">
                                                {req.state === "WAIT" ? (
                                                    <div className="flex flex-col items-center space-y-1">
                                                        ...
                                                        <div className="font-semibold whitespace-nowrap text-xs">{req.updateDate || "-"}</div>
                                                    ...
                                                    <div className="text-xs font-semibold">처리 완료</div>
                                                ) : (
