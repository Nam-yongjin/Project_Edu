...
public interface DemonstrationTimeRepository extends JpaRepository<DemonstrationTime, Long> { // 실증 시간 레포지토리 (상세시간)
	...
	// 리포지토리에서 생성자 호출해서 객체를 리스트에
	@Query("SELECT new com.EduTech.dto.demonstration.DemonstrationTimeDTO(dt.demDate, dt.state,dt.demonstration.demNum) FROM DemonstrationTime dt WHERE dt.demDate IN:demDate")

	// startDate 랑 endDate사이의 날짜를 저장한 demDate를 가져와 in사용하여 받아옴. // 담음.
	List<DemonstrationTimeDTO> selectDemTime(@Param("demDate") List<LocalDate> demDate);
