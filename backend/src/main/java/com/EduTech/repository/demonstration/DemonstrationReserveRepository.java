...
public interface DemonstrationReserveRepository
	...
	// 백쪽에서만 사용할거면 dto대신 엔티티로 받아도 문제없다. (다중)
	...
	@Modifying // demonstration reserve 테이블 상태값 변경 쿼리문
	@Transactional
	...
	@Query("SELECT COUNT(r) > 0 FROM DemonstrationReserve r WHERE r.member.memId = :memId AND r.demonstration.demNum = :demNum AND r.state!=:state")
