...
public interface DemonstrationRequestRepository extends JpaRepository<DemonstrationRequest, Long>{
	...
	int updateDemResChangeStateReq(@Param("state") DemonstrationState state,@Param("demRevNum") Long demRevNum,@Param("type") RequestType type,@Param("updateState") DemonstrationState updateState);
