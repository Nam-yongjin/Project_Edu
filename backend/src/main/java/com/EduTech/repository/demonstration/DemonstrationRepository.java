...
public interface DemonstrationRepository extends JpaRepository<Demonstration, Long> { // 실증 상품 관련 레포지토리
	...
	@Query("SELECT new com.EduTech.dto.demonstration.DemonstrationPageListDTO(d.demNum, d.demName, d.demMfr, d.itemNum, reg.state, c.companyName,reg.expDate) "
			+ "FROM Demonstration d " + "JOIN DemonstrationRegistration reg ON d.demNum = reg.demonstration.demNum "
			+ "JOIN Company c ON reg.member.memId = c.member.memId " + "WHERE reg.state = :state")
	...
	@Query("SELECT new com.EduTech.dto.demonstration.DemonstrationPageListDTO(d.demNum, d.demName, d.demMfr, d.itemNum, reg.state, c.companyName,reg.expDate) "
			+ "FROM Demonstration d " + "JOIN DemonstrationRegistration reg ON d.demNum = reg.demonstration.demNum "
			+ "JOIN Company c ON reg.member.memId = c.member.memId "
			+ "WHERE d.demName LIKE %:search% AND reg.state = :state")
	Page<DemonstrationPageListDTO> selectPageDemName(Pageable pageable, @Param("search") String search,
	...
	@Query("SELECT new com.EduTech.dto.demonstration.DemonstrationPageListDTO(d.demNum, d.demName, d.demMfr, d.itemNum, reg.state, c.companyName,reg.expDate) "
			+ "FROM Demonstration d " + "JOIN DemonstrationRegistration reg ON d.demNum = reg.demonstration.demNum "
			+ "JOIN Company c ON reg.member.memId = c.member.memId "
			+ "WHERE d.demMfr LIKE %:search% AND reg.state = :state")
	Page<DemonstrationPageListDTO> selectPageDemMfr(Pageable pageable, @Param("search") String search,
			@Param("state") DemonstrationState state);

	// 실증 상품들을 페이지 별로 가져오는 쿼리문 (실증 장비 신청 목록 페이지) 기업명 기준
	@Query("SELECT new com.EduTech.dto.demonstration.DemonstrationPageListDTO(d.demNum, d.demName, d.demMfr, d.itemNum, reg.state, c.companyName,reg.expDate) "
			+ "FROM Demonstration d " + "JOIN DemonstrationRegistration reg ON d.demNum = reg.demonstration.demNum "
			+ "JOIN Company c ON reg.member.memId = c.member.memId "
			+ "WHERE c.companyName LIKE %:search% AND reg.state = :state")
	Page<DemonstrationPageListDTO> selectPageDemCompanyName(Pageable pageable, @Param("search") String search,
			@Param("state") DemonstrationState state);

	// 실증 상품들을 페이지 별로 가져오는 쿼리문 (실증 장비 신청 상세 페이지)
	@Query("SELECT new com.EduTech.dto.demonstration.DemonstrationDetailDTO(d.demNum, d.demName, d.demInfo,d.demMfr, d.itemNum,reg.expDate,d.category,c.companyName) FROM Demonstration d, DemonstrationRegistration reg, Company c WHERE d.demNum = reg.demonstration.demNum AND d.demNum=:demNum AND reg.member.memId=c.member.memId")
	...
	int updateDem(@Param("demName") String demName, @Param("demMfr") String demMfr, @Param("itemNum") Long itemNum,
			...
			@Param("category") DemonstrationCategory category);
