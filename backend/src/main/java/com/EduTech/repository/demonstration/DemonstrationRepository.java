...
public interface DemonstrationRepository extends JpaRepository<Demonstration, Long> { // 실증 상품 관련 레포지토리
	...
	@Query("SELECT new com.EduTech.dto.demonstration.DemonstrationPageListDTO(demNum, demName, demMfr, itemNum) FROM Demonstration WHERE demName LIKE %:search%")
	...
	@Query("SELECT new com.EduTech.dto.demonstration.DemonstrationPageListDTO(demNum, demName, demMfr, itemNum) FROM Demonstration WHERE demMfr LIKE %:search%")
