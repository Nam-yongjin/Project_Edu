...
public interface EventUseRepository extends JpaRepository<EventUse, Long>{
	...
	List<EventBanner> findByMember_MemId(String memId);	// 회원별 신청목록 조회(리스트 형태)

	Page<EventBanner> findByMember_MemId(String memId, Pageable pageable);	// 회원별 신청목록 조회(페이지 형태)

	List<EventBanner> findByEvent_eventNum(Long eventNum);	// 신청한 프로그램의 모든정보 조회

	Page<EventBanner> findByMember(Member member, Pageable pageable);	// Member기준으로 예약내역 조회
	...
	// member 함께 로딩해서 DTO 변환시 NPE(널포인터예외) 방지
		...
		List<EventBanner> findWithMemberByEvent_EventNum(@Param("eventNum") Long eventNum);
	// ---------- 탈퇴 회원 프로그램 신청 내역 삭제 ----------
	...
	List<EventBanner> findAllByMember_MemId(String memId);
