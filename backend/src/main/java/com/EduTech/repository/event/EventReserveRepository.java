...
public interface EventReserveRepository extends JpaRepository<EventReserve, Long>{
	...
	List<EventReserve> findByMember_Mid(String memId);	// 회원별 신청목록 조회(리스트 형태)
	
	Page<EventReserve> findByMember_Mid(String memId, Pageable pageable);	// 회원별 신청목록 조회(페이지 형태)
	...
	void deleteByMember_Mid(String memId); // 특정 회원 행사 신청 내역 모두 삭제 (DB상에서 바로 DELETE쿼리 실행)
	
	List<EventReserve> findAllByMember_Mid(String memId);	// 특정 회원 행사 신청 내역을 모두 조회(필요시 사용하면 됨, 현재는 필요 없음)
