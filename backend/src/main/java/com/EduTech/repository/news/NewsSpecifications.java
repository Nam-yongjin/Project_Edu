...
public class NewsSpecifications {
	//NewsSearchDTO를 받아서 Specification 생성 //타입과 키워드를 조건으로 사용
	public static Specification<News> fromDTO(NewsSearchDTO dto) {
		return Specification
				.where(searchFilter(dto.getSearchType(), dto.getKeyword()));
	}
	//사용자 검색 필터(단일 조건 - return)
	//검색 조건을 필드별로 생성해주는 메서드
	private static Specification<News> searchFilter(String searchType, String keyword) {
