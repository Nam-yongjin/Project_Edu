...
public interface FacilityReserveRepository extends JpaRepository<FacilityReserve, Long> {
	...
	boolean existsByFacility_FacilityNumAndFacDateAndStartTimeLessThanAndEndTimeGreaterThanAndStateIn(Long facilityNum,
			LocalDate facDate, LocalTime endTime, LocalTime startTime, List<FacilityState> states);
	...
	@Query("""
			...
			""")
	List<FacilityReserve> searchForAdmin(@Param("state") FacilityState state, @Param("from") LocalDate from,
	...
	@Query("""
			...
			""")
	int updateStateIfCurrent(@Param("facRevNum") Long facRevNum, @Param("newState") FacilityState newState,
	...
	@Query("""
			...
			""")
	int cancelReservation(@Param("facRevNum") Long facRevNum, @Param("isAdmin") boolean isAdmin,
	...
	// 회원 탈퇴할때, 공간 예약 완료 상태이면 회원 탈퇴 못하도록 구현하기 위한 쿼리문
	@Query("SELECT COUNT(e) > 0 FROM FacilityReserve e WHERE e.member.memId = :memId AND e.state = 'APPROVED'")
