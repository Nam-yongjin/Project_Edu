...
public interface DemonstrationService {
		...
		PageResponseDTO<DemonstrationRentalListDTO> getAllDemRental(String memId, String search,int pageCount); // 회원이 신청한 물품 대여 조회 페이지 조회 기능 (검색도 같이 구현할 것임.)
		PageResponseDTO<DemonstrationPageListDTO> getAllDemList(int pageCount); // 실증 장비신청 페이지 (실증 물품 리스트 목록)
